...
def chart_section(df):
    ...
    with tab_line:
        ...
        y_options = [col for col in numeric_cols if col != x]

        if not y_options:
            st.warning("Select a different X-axis to enable Y-axis.")
        else:
            y = st.selectbox("Y-axis (Numeric)", y_options, key="line_y")
    ...
    with tab_bar:
        ...
        if not y_options:
            ...
            y = st.selectbox("Y-axis (Numeric)", y_options, key="bar_y")
    ...
    with tab_area:
        ...
        if not y_options:
            ...
            y = st.selectbox("Y-axis (Numeric)", y_options, key="area_y")
    ...
    with tab_scatter:
        ...
        if not y_options:
            ...
            y = st.selectbox("Y-axis (Numeric)", y_options, key="scatter_y")
            chart = (
                alt.Chart(df)
                .mark_circle(size=60)
                .encode(x=x, y=y, tooltip=all_cols)
                .interactive()
            )
    ...
    with tab_pie:
        ...
        if not categorical_cols:
            ...
            x = st.selectbox("Category", categorical_cols, key="pie_x")
